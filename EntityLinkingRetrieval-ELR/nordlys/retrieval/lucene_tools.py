...
from org.apache.lucene.search.spans import SpanNearQuery, SpanWeight, Spans
from org.apache.lucene.search.spans import SpanTermQuery

# newly added for lucene 6
from org.apache.lucene.search.spans import Spans
...
class Lucene(object):
    ...
    def get_lucene_query(self, query, field=FIELDNAME_CONTENTS):
        ...
        return QueryParser(field,self.get_analyzer()).parse(query)
    ...
    def get_doc_phrase_freq(self, phrase, field, slop, ordered):
        # this function works for Pylucene 6
        """
        ...
        # create span weight (for lucene 6)
        self.open_searcher()
        sw=span_near_query.createWeight(self.searcher,False)
        leaves = self.reader.getContext().leaves()
        
        doc_phrase_freq = {}
        # iterates over all atomic readers
        for atomic_reader_context in leaves:
            spans = sw.getSpans(atomic_reader_context, SpanWeight.Postings.POSITIONS)
            if spans is None:
               continue
            
            while spans.nextDoc()!=DocIdSetIterator.NO_MORE_DOCS:
                  lucene_doc_id=spans.docID()
                  doc_id=atomic_reader_context.reader().document(lucene_doc_id).get(self.FIELDNAME_ID)
                  if doc_id not in doc_phrase_freq:
                     doc_phrase_freq[doc_id]=0
                     
                  while spans.nextStartPosition()!=Spans.NO_MORE_POSITIONS:
                        #print 'position=%d'%(spans.startPosition())
                        doc_phrase_freq[doc_id]+=1
            
        return doc_phrase_freq
        
    def get_doc_phrase_freq_old(self, phrase, field, slop, ordered):
        # this function only works for PyLucene < 6
